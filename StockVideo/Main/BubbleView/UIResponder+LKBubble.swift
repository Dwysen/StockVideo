...
extension NSObject{
    ...
    // MARK: -  获取默认的显示成功的泡泡信息对象，可以在此基础之上自定义
    ///
    /// - returns: 泡泡信息描述对象
    func getDefaultRightBubbleInfo() -> LKBubbleInfo {
        ...
        info.iconAnimation = {(layer: CAShapeLayer) -> Void in
            ...
            circlePath.addArc(withCenter: CGPoint(x: layer.frame.size.width / 2 , y: layer.frame.size.height / 2), radius: layer.frame.size.width / 2 - CGFloat(STROKE_WIDTH), startAngle: CGFloat(0 * CGFloat.pi) / 180, endAngle: CGFloat(360 * CGFloat.pi) / 180.0, clockwise: false)
            // 创建外部透明圆形的图层
            ...
            // 绘制大半圆
            path.addArc(withCenter: CGPoint(x: layer.frame.size.width / 2 , y: layer.frame.size.height / 2), radius: layer.frame.size.width / 2 - STROKE_WIDTH, startAngle: CGFloat(67 * CGFloat.pi / 180), endAngle: CGFloat(-158 * CGFloat.pi / 180), clockwise: false)
            // 绘制对号第一笔
...
//        info.iconColor = UIColor(red: 0 / 255.0, green: 205 / 255.0, blue: 0, alpha: 1)
        info.iconColor = Common.commonYellow
    ...
    // MARK: - 获取默认的显示加载中的泡泡信息对象，可以在此基础之上自定义
    ...
    func getDefaultRoundProgressBubbleInfo() -> LKBubbleInfo {
        ...
        info.iconAnimation = {(layer: CAShapeLayer) -> Void in
            ...
            progressPath.addArc(withCenter: CGPoint(x: layer.frame.size.width / 2 , y: layer.frame.size.height / 2), radius: layer.frame.size.width / 2 - STROKE_WIDTH, startAngle: CGFloat(0 * CGFloat.pi / 180), endAngle: CGFloat(360 * CGFloat.pi / 180), clockwise: true)
            ...
            progressRotateAnimation.toValue = NSNumber(value: Double.pi / 180 * 360)
    ...
    func showRoundProgressWithTitle(title: String, autoCloseTime:CGFloat) -> Void {
        let info: LKBubbleInfo = self.getDefaultRoundProgressBubbleInfo()
        info.title = title
...
//        let image = UIImage.init(named: "wait")
//        iconArr.append(image!)
//        info.iconArray = iconArr
        
        info.title = title
        LKBubbleView.defaultBubbleView().showWithinfo(info:  info, autoCloseTime:  autoCloseTime)
    }
    
    func showWaitImageWithTitle(title: String ,autoCloseTime: CGFloat) -> Void {
        let info: LKBubbleInfo = self.getDefaultErrorBubbleInfo()
    ...
    //MARK: - 获取默认的显示错误的泡泡信息对象，可以在此基础之上自定义
    ///
    /// - returns: 泡泡信息描述对象
    func getDefaultErrorBubbleInfo() -> LKBubbleInfo {
        ...
        info.iconAnimation = {(layer: CAShapeLayer) -> Void in
            // 绘制外部透明的圆形
            let circlePath: UIBezierPath = UIBezierPath()
            circlePath.addArc(withCenter: CGPoint(x: layer.frame.size.width / 2 , y: layer.frame.size.height / 2), radius: layer.frame.size.width / 2 - STROKE_WIDTH, startAngle: CGFloat(0 * CGFloat.pi) / 180, endAngle: CGFloat(360 * CGFloat.pi) / 180.0, clockwise: false)
            // 创建外部透明圆形的图层
            let alphaLineLayer = CAShapeLayer()
            alphaLineLayer.path = circlePath.cgPath// 设置透明圆形的绘图路径
            alphaLineLayer.strokeColor = UIColor(cgColor: layer.strokeColor!).withAlphaComponent(0.1).cgColor// 设置图层的透明圆形的颜色
            ...
            // 绘制大半圆
            leftPath.addArc(withCenter: CGPoint(x: layer.frame.size.width / 2 , y: layer.frame.size.height / 2), radius: layer.frame.size.width / 2 - STROKE_WIDTH, startAngle: CGFloat(-43 * CGFloat.pi / 180), endAngle: CGFloat(-315 * CGFloat.pi / 180), clockwise: false)
            ...
            // 绘制大半圆
            rightPath.addArc(withCenter: CGPoint(x: layer.frame.size.width / 2 , y: layer.frame.size.height / 2), radius: layer.frame.size.width / 2 - STROKE_WIDTH, startAngle: CGFloat(-128 * CGFloat.pi / 180), endAngle: CGFloat(133 * CGFloat.pi / 180), clockwise: true)
        ...
        info.bubbleSize = CGSize(width: 300 , height: 100)
        return info
    }
