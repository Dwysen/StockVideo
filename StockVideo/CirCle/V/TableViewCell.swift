...
//            利用Kingfisher加载图片
            imageV.kf.setImage(with: URL(string:imageArr[index] as! String)!, placeholder:nil, options: nil, progressBlock: nil, completionHandler: nil)
            imageV.tag = index
...
//        CGFloat(num/3+1)*kHeight+kSpace*CGFloat(num/3)
        ...
        images.snp.updateConstraints { (ls) in
            ls.height.equalTo(height).priority(666)
        }
    }
